...
TEAS = {
    ...
    "Black Tea": 40,
    "Sulemani Tea": 45,
    "English Breakfast Tea 50g": 60,
    "Ginger Tea with Honey": 55,
    "Ginger Tea with Lemon": 55,
    "Peshawari Kehwa": 70,
    "Kashmiri Chai": 80,
    "Classical Tea": 50
}
...
def save_order(order_id, user_name, tea, quantity, price, date, time):
    row = {
        'OrderID': order_id,
        'UserName': user_name,
        'TeaType': tea,
...
def chat():
    ...
    # System prompt strictly enforcing language
    system_prompt = f"""
You are Sana, a warm and friendly female assistant at Vital Tea Café.

Available teas: {', '.join(TEAS.keys())}

Rules:
- Greet politely and ask for name only if not known yet.
- Offer 2–3 tea options at a time.
- Ask if they want more options.
- Allow multiple items and quantities (e.g., "2 Black Tea").
- Confirm order before placing.
- Once name and full order are confirmed, call the place_order tool.

CRITICAL LANGUAGE RULE:
- If the conversation started in English → respond ONLY in proper English. Never use Urdu script or Roman Urdu.
- If the conversation started in Urdu script → respond ONLY in proper Urdu script. Never use English or Roman Urdu.
- Never mix languages. Never use Roman Urdu under any circumstances.

Be polite, natural, and patient.
"""
    ...
    tools = [{
        ...
        "function": {
            "name": "place_order",
            "description": "Save the complete order when user has confirmed name and all tea items with quantities.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    try:
            ...
            for call in msg.tool_calls:
                if call.function.name == "place_order":
                    ...
                    for item in args["orders"]:
                        ...
                        total += price_per * qty
                        save_order(order_id, user_name, tea, qty, price_per, now.date(), now.strftime("%H:%M:%S"))

                    if lang == "english":
                        reply = f"Thank you {user_name}! Your order #{order_id} has been placed successfully. Total amount: {total} PKR. Enjoy your tea! ☕"
                    else:
                        reply = f"شکریہ {user_name} صاحبہ! آپ کا آرڈر نمبر {order_id} کامیابی سے موصول ہو گیا ہے۔ کل رقم: {total} روپے۔ اپنی چائے کا مزہ لیں! ☕"
        ...
        print("Error:", e)
        error_msg = "معاف کیجیے، تکنیکی مسئلہ ہے۔" if lang == "urdu" else "Sorry, technical issue."
...
def voice_chat():
    ...
    try:
        ...
        # Same system prompt and tools as text chat
        system_prompt = f"""
You are Sana, a warm and friendly female assistant at Vital Tea Café.
